...
function SandpackFileWatcher({ onLiveFilesChange }) {
  ...
  const { activeProejct, updateProjectFiles } = useAppContext();
  const activeProjectRef = useRef(activeProejct);
  useEffect(() => {
    activeProjectRef.current = activeProejct;
  }, [activeProejct]);
...
const PreviewPannel = ({ project, activeFile, showCode }) => {
  ...
  //Detect dependices from import statement using liveFiles
